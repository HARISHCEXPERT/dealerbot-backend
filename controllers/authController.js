...
// ✅ Step 1: Meta se code aaya -> access token lo -> WABA ID save karo
  ...
  try {
    ...
    const tokenRes = await axios.get("https://graph.facebook.com/v19.0/oauth/access_token", {
      params: {
        ...
        code: code,
        redirect_uri: `${BACKEND_URL}/api/auth/meta/callback?clientId=${clientId}`
      }
    });

    const accessToken = tokenRes.data.access_token;

    // WABA ID fetch karo is token se
    ...
    // Client Supabase mein update karo
...
// ✅ Step 2: Client ka onboard status check karo
...
// ✅ Step 3: Onboard URL generate karo (frontend isko call karega)
const getOnboardUrl = async (req, res) => {
  ...
  try {
    ...
    const redirectUri = encodeURIComponent(
      `${BACKEND_URL}/api/auth/meta/callback?clientId=${clientId}`
    );
